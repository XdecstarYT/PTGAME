...
export class InteriorEditor {
  ...
  // deck 0 is the ground floor (model.floorPlan, has the door-zone row);
  // deck 1 is the upper deck of a double-decker (model.upperFloorPlan) -
  // real double-decker doors only open onto the lower deck, so the door row
  // only ever applies to deck 0.
  setModel(model, chassis, deck = 0) {
    this._model = model;
  ...
  _floorPlan() { return this._deck === 1 ? this._model.upperFloorPlan : this._model.floorPlan; }

  render() {
    if (!this._model || !this._chassis) return;
    const floorPlan = this._floorPlan();
    if (!floorPlan) return;
    const { doorZonesActive } = this._model;
  ...
  _paint(r, c) {
    this._floorPlan()[r][c] = this.brush;
